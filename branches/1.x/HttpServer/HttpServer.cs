...
namespace HttpServer
{
    ...
    public class HttpServer
    {
		...
		{
            _requestQueue.SafeIncrementRequestCount();
			ProcessRequest(context, request);
            _requestQueue.SafeDecrementRequestCount();
		}
		...
		public void Start(IPAddress address, int port)
		{
			...
			if (port < 0)
				throw new ArgumentException("Port cannot be negative.");
        ...
        public void Start(IPAddress address, int port, X509Certificate certificate, SslProtocols protocol,
            RemoteCertificateValidationCallback clientCertCallback, bool requireClientCerts)
        {
            if (address == null)
                throw new ArgumentNullException("address");
            if (port < 0)
