...
namespace HttpServer.HttpModules
{
    ...
    public class ResourceFileModule : HttpModule
    {
		...
		/// </usage>
		public void AddResources(string toUri, Assembly fromAssembly, string fromNamespace)
		{
		...
		{
			if(!request.Uri.AbsolutePath.EndsWith("*") && _resourceManager.ContainsResource(request.Uri.AbsolutePath))
				return true;
		...
		#region Test GetResourceStream

		[Fact]
        private static void TestProcess()
        {
            ResourceFileModule module = new ResourceFileModule();
			module.AddResources("/base/", module.GetType().Assembly, "HttpServer.Rendering");
